...
const INITIAL_STATE: ProductState = { isLoggedIn: undefined, products: TempDataService.products, isLoading: false, isDeleting: false };
...
export function productsReducer(state: ProductState = INITIAL_STATE, action: any) {
  ...
  switch (action.type) {
    ...
    case ProductActions.DELETE_PRODUCT_LOADING:
      return tassign(state, { isDeleting: true });
    ...
    case ProductActions.DELETE_PRODUCT_SUCCESS:
      ...
      //return tassign(state, { products: action.payload, isDeleting: false });

    case ProductActions.DELETE_PRODUCT_FAILURE:
      return tassign(state, { isDeleting: false });
